        ...
        return (
            <div className="loginform">
                ...
                <h1 className='loginform'>Register</h1>
                    ...
                    <div>{hasError ? 'Email taken or invalid email format. E.g. example@g.com' : ''} </div>
                    <div className='logindiv'>
                        <label htmlFor='password'>Password:</label>
                        <input className='logininputs' type='password' value={ password } onChange={ ev => this.setState({ password: ev.target.value }) } /> 
                    </div>
                    <div>{password.length < 6 ? 'Must be 6 or more characters' : ''}</div>
                    {/* <div>{hasError ? 'Password has to be minimum of 6 characters long' : ''} </div> */}
                    {/* <button className='loginbutton'>Register</button> */}
                    <button disabled={ !email || !password || !name || name.length < 6 || password.length < 6 ? 'disabled' : '' } className='loginbutton'>Register</button>
                </form>
            </div>
        );
    }
